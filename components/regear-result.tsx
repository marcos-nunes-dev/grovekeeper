...
function PriceDisplay({ value, formattedValue, isReliable }: { value: number, formattedValue: string, isReliable: boolean }) {
  if (value === 0) {
    return (
      ...
      {!isReliable && (
        <Tooltip delayDuration={300}>
          <TooltipTrigger asChild>
            ...
            >
              <HelpCircle className="w-4 h-4 text-zinc-400" />
              <span className="sr-only">Price reliability info</span>
            </button>
          </TooltipTrigger>
          <TooltipContent side="top">
            <p>This price might not be accurate</p>
          </TooltipContent>
        </Tooltip>
      )}
    </div>
